...
class Batch06(
    ...
    @Bean
    fun batch06Job(): Job {
        return JobBuilder("batch06Job", jobRepository)
            .start(batch06Step())
            .build()
    }
    ...
    @Bean
    @JobScope
    fun batch06Step(): Step {
        return StepBuilder("batch06Step", jobRepository)
            .chunk<ReadJpaEntity, ReadJpaEntity>(CHUNK_SIZE, transactionManager)
            .reader(batch06QueryDslPagingReader())
            .writer(batch06Writer())
            .build()
    }

    @Bean
    fun batch06QueryDslPagingReader(): QuerydslNoOffsetPagingItemReader<ReadJpaEntity> {
    ...
    @Bean
    fun batch06Writer(): ItemWriter<ReadJpaEntity> {
